...
export function SettingsScreen() {
  ...
  const handleExport = () => {
    ...
    const filename = `kirijo-wishes-${new Date().toISOString().slice(0,10)}.json`;

    // Try Native Android Plugin first
    if (typeof window !== 'undefined' && (window as any).KirijoFileManager) {
      try {
        (window as any).KirijoFileManager.exportData(jsonStr, filename);
        showToast('✦ Exporting to mobile storage...');
        return;
      } catch (e) {
