...
import { Search as SearchIcon, ChevronDown, Sparkles, Navigation2, Heart, Activity, Map as MapIcon, MoreHorizontal, PenLine, Lock, Star, Bell } from 'lucide-react';
...
export function WishCard({ wish, index, onClick }: { wish: Wish, index: number, onClick: () => void }) {
  ...
  return (
    <motion.div 
      ...
      transition={{ delay: Math.min(index * 0.05, 0.3), ease: [0.25, 0.46, 0.45, 0.94] }}
      onClick={onClick}
      className={cn(
        "bg-surface border border-border2 rounded-2xl p-4 mb-3 relative overflow-hidden transition-all active:scale-[0.985] active:bg-surface2 cursor-pointer",
        wish.important ? "" : ""
      )}
    >
      <div className="absolute top-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-accent/25 to-transparent" />
      {wish.important && <div className="absolute top-0 right-0 w-[3px] bottom-0 rounded-r-2xl bg-gradient-to-b from-warning to-transparent" />}

      <div className="flex items-start gap-3">
        <div className="mt-0.5 shrink-0 opacity-80"><Icon size={20} /></div>
        <div className="flex-1 min-w-0">
          <div className="font-head text-[18px] font-bold leading-tight mb-1 text-text">{wish.title}</div>
          {wish.content && <div className="text-[13px] text-text-sub leading-snug line-clamp-2 mb-2.5">{wish.content}</div>}
          ...
          <div className="flex flex-wrap gap-1.5 mb-2">
            ...
            {wish.important && <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-bold tracking-wider uppercase font-head bg-warning/10 border border-warning/20 text-warning"><Star size={10} fill="currentColor" /> IMPORTANT</span>}
            {wish.reminder && <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-bold tracking-wider uppercase font-head bg-info/10 border border-info/20 text-info"><Bell size={10} /> REMINDER</span>}
          </div>
          ...
          <div className="flex items-center justify-between mt-1">
            ...
            {wish.reminder && <div className="text-[11px] text-info flex items-center gap-1"><Bell size={10} /> {new Date(wish.reminder).toLocaleDateString()}</div>}
          </div>
        </div>
      </div>
    </motion.div>
